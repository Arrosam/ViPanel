    ...
    aiTools: {
        console: {
            ...
            loginMode: {
                ...
                devicecode: 'デバイスコードでログイン',
                devicecodeNote: '任意の端末で以下のリンクを開き、表示された使い捨てコードを入力します',
            },
            loginFollowOutput: '下のターミナル出力の指示に従って認証を完了してください',
            install: 'インストール',
            installTitle: 'エージェントのインストール',
            installStart: 'インストール開始',
            installRunning: 'インストール中です。数分かかる場合があります',
            installOk: 'インストール完了',
            installFail: 'インストールに失敗しました。上の出力を確認してください',
            done: '完了',
            installWarn: 'このサーバーに root 権限でソフトウェアをインストールします。上記の入手元を信頼できる場合のみ続行してください。',
            agentManage: 'エージェント管理',
            agentManageHint: 'このサーバーで使えるコーディングエージェントと、ログイン中のアカウントを管理します。',
            addAccount: '別のアカウントを追加',
            saveAccount: '現在のアカウントを保存',
            accountSaved: 'アカウントを保存しました',
            accountSwitched: 'アカウントを切り替えました',
            accountSwitchedRestart: '切り替えました — 実行中のセッションは再起動が必要です',
            ...
            brandTagline: '個人向けのオープンソースサーバー管理パネルです。アプリケーション、ウェブサイト、コンテナ、データベースを管理し、さらに操作を代行する AI アシスタントを内蔵しています。',
            brandPoint1: 'AI アシスタントはコンテナではなくホスト上で実行されるため、マシン全体を管理できます',
            brandPoint2: 'すべての変更はブラウザ上で平易な言葉で確認されます。生の JSON は不要です',
            brandPoint3: 'シングルノード、オープンソース、セルフホスト',
            brandFork: '1Panel の改変版 · GPL-3.0 · FIT2CLOUD とは提携していません',
