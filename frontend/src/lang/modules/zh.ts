...
const message = {
    commons: {
        ...
        login: {
            username: '用户名',
            password: '密码',
            errorAuthInfo: '用户名或密码不正确，请重试。',
            errorMfaInfo: '验证码无效，请重试。',
            captchaHelper: '验证码',
            errorCaptcha: '验证码不正确。',
            mfaTitle: 'MFA 验证',
            mfaCode: 'MFA 验证码',
            ...
            passkeyNotSupported: '当前浏览器或环境不支持通行密钥，请确认已绑定域名并通过 HTTPS 访问。',
            passkeyToPassword: '通行密钥不可用？改用密码登录',
            licenseHelper: '《飞致云社区软件许可协议》',
            errorAgree: '请点击同意社区软件许可协议',
            agreeTitle: '服务协议',
            agreeContent:
                '为更好保障您的合法权益，请阅读并同意以下协议 &laquo; <a href="https://www.fit2cloud.com/legal/licenses.html" target="_blank"> 飞致云社区软件许可协议 </a> &raquo;',
            logout: '退出登录',
        },
        rule: {
            username: '请输入用户名',
    ...
    aiTools: {
        console: {
            console: '控制台',
            empty: '暂无会话',
            cwdPlaceholder: '工作目录，例如 /root/project',
            sessions: '会话',
            newSession: '新建会话',
            rename: '重命名',
            pickHint: '选择一个会话，或新建一个',
            restart: '重启 Agent',
            restarted: '已重启，对话保留',
            finish: '结束会话',
            finishHint: '将终止该会话的进程。对话记录仍保留在磁盘上。',
            poolState: '运行中 {0} / {1}',
            chatEmpty: '该会话暂无对话',
            inputHint: '输入消息，按 Enter 发送',
            thinking: '思考中',
            send: '发送',
            stop: '停止',
            terminal: '终端',
            ...
            loginMode: {
                claudeai: 'Claude 订阅',
                claudeaiNote: '使用你的 Pro / Max 订阅额度',
                console: 'Anthropic Console',
                consoleNote: 'API 按量计费，不占用订阅额度',
                devicecode: '使用设备码登录',
                devicecodeNote: '在任意设备上打开下方链接，输入终端给出的一次性验证码',
            },
            loginFollowOutput: '按下方终端输出里的提示完成授权',
            install: '安装',
            installTitle: '安装 Agent',
            installStart: '开始安装',
            installRunning: '正在安装，可能需要几分钟',
            installOk: '安装成功',
            installFail: '安装失败，请看上方输出',
            done: '完成',
            installWarn: '这会以 root 身份往这台服务器上安装软件。请确认你信任上面写明的来源再继续。',
            agentManage: 'Agent 管理',
            agentManageHint: '管理这台服务器上可用的编码 Agent，以及各自登录的账号。',
            addAccount: '添加另一个账号',
            saveAccount: '记住当前账号',
            accountSaved: '账号已保存',
            accountSwitched: '已切换账号',
            accountSwitchedRestart: '已切换 —— 正在运行的会话需重启才会生效',
            ...
            permAsk: '返回终端',
            permAlways: '始终允许',
            brandTagline: '一款面向个人的开源服务器管理面板。管理应用、网站、容器和数据库，并内置可代为执行操作的 AI 助手。',
            brandPoint1: 'AI 助手运行在宿主机上而非容器内，可管理整台机器',
            brandPoint2: '每次变更均在浏览器中以自然语言确认，无需查看原始 JSON',
            brandPoint3: '单节点、开源、可自托管',
            brandFork: '1Panel 的修改版本 · GPL-3.0 · 与飞致云无隶属关系',
            permFirstUse: '同时授权本会话使用「{0}」板块。',
            permConfirmAgain: '再次点击以确认',
            permDanger: '危险操作，执行后不可撤销。',
            permRawInput: '原始参数',
            permModuleHint: '授权后，本会话内该板块的只读操作不再询问；变更和删除仍会逐一确认。不跨会话保留。',
            permOpCount: '{0} 个操作',
            permDestructiveCount: '其中 {0} 个为删除类',
            mcpTitle: '面板控制（MCP）',
            mcpHint: '让 Agent 能操作此面板：应用、网站、证书、容器。每次变更仍需你先确认。',
            mcpMissing: 'vipanel-mcp 未安装，面板控制不可用。',
            mcpState: '已就绪 {0} 个操作',
            agentAsks: 'Agent 有问题要问',
            permOff: '权限代理未启用',
            permOffHint: '工具调用不经面板确认，Agent 将直接执行。',
            files: '文件',
            fileUp: '上级目录',
            fileHome: '会话目录',
            fileMkdir: '新建文件夹',
            ...
            fileEmpty: '空目录',
            history: '历史',
            historyEmpty: '没有可重开的会话',
            settings: '设置',
            poolSize: 'Agent 并发实例上限',
            poolHint: '会话数量不限。超过上限时，最久未使用的会话将休眠，重新打开时恢复原对话。',
            harnesses: '运行框架',
            capChat: '结构化聊天',
            capTermOnly: '仅终端',
            save: '保存',
            ...
            restore: '恢复',
            toRecycle: '移入回收站',
            toRecycleHint: '并非永久删除——会移入回收站，仍可恢复。',
            reconnectTerm: '重连终端',
            reconnectTermHint: '将替换当前 shell，正在运行的命令会被中断。聊天和 Agent 不受影响。',
            newSessionHere: '在当前目录新建会话',
            createHere: '在此新建',
            noSubdir: '无子目录',
            status: {
                idle: '空闲',
