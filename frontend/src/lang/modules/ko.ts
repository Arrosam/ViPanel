    ...
    aiTools: {
        console: {
            ...
            loginMode: {
                ...
                devicecode: '기기 코드로 로그인',
                devicecodeNote: '아무 기기에서나 아래 링크를 열고 일회용 코드를 입력하세요',
            },
            loginFollowOutput: '아래 터미널 출력의 안내에 따라 인증을 완료하세요',
            install: '설치',
            installTitle: '에이전트 설치',
            installStart: '설치 시작',
            installRunning: '설치 중입니다. 몇 분 걸릴 수 있습니다',
            installOk: '설치 완료',
            installFail: '설치 실패 — 위 출력을 확인하세요',
            done: '완료',
            installWarn: '이 서버에 root 권한으로 소프트웨어를 설치합니다. 위에 표시된 출처를 신뢰하는 경우에만 진행하세요.',
            agentManage: '에이전트 관리',
            agentManageHint: '이 서버에서 사용할 수 있는 코딩 에이전트와 로그인 계정을 관리합니다.',
            addAccount: '다른 계정 추가',
            saveAccount: '현재 계정 저장',
            accountSaved: '계정이 저장되었습니다',
            accountSwitched: '계정을 전환했습니다',
            accountSwitchedRestart: '전환됨 — 실행 중인 세션은 다시 시작해야 적용됩니다',
            removeAccountConfirm: '저장된 이 계정을 삭제할까요? 현재 로그인 상태에는 영향이 없습니다.',
            remove: '삭제',
            ready: '사용 가능',
            agents: '에이전트',
            proxyTitle: '네트워크 프록시',
            proxyHint: '콘솔의 아웃바운드 트래픽을 프록시로 보냅니다. 이 서버에서 Anthropic 또는 OpenAI에 직접 연결되지 않을 때(지역 차단 등) 사용하세요.',
            proxyExample: '프록시/VPN 클라이언트가 여는 로컬 HTTP 포트. 예: http://127.0.0.1:7890. SOCKS는 지원되지 않습니다 — Claude Code와 Codex 모두 사용할 수 없습니다.',
            proxyTest: '연결 테스트',
            proxyReachOk: '연결됨',
            ...
            brandTagline: '개인용 오픈소스 서버 관리 패널입니다. 애플리케이션, 웹사이트, 컨테이너, 데이터베이스를 관리할 수 있으며, 작업을 직접 수행하는 내장 AI 어시스턴트를 제공합니다.',
            brandPoint1: 'AI 어시스턴트는 컨테이너가 아닌 호스트에서 실행되어 전체 머신을 관리할 수 있습니다',
            brandPoint2: '모든 변경 사항은 브라우저에서 일반 언어로 확인됩니다 — 원시 JSON이 아닙니다',
            brandPoint3: '단일 노드, 오픈소스, 자체 호스팅',
            brandFork: '1Panel의 수정된 버전 · GPL-3.0 · FIT2CLOUD와 제휴하지 않음',
