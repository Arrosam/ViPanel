...
# 且机器上没有僵尸进程。
# 跨度用整除算，所以 23h59m 会被算成 23 —— 这条咬过两次。
#
# 采样是每小时一次，而 systemd 定时器会往前漂（24 次采样累计漂了约 40 秒），
# 所以「第 24 次采样」的实际时刻是 T0+23h59m22s。verdict 定在 24h 或 24h30m
# 都只会看到那一次采样，算出来还是 23。
#
# 要让它满，verdict 至少要定在 **26h**：那时最后一次采样是 T0+25h 附近，
# 跨度稳稳超过 24h。不要去改下面的整除逻辑——那是在改判据本身。
