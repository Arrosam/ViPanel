...
const message = {
    ...
    aiTools: {
        console: {
            ...
            permAsk: '退回终端',
            permAlways: '总是允许',
            brandTagline: '一个人管一台服务器需要的东西：应用、网站、容器、数据库，外加一个能替你动手的 AI 控制台。',
            brandPoint1: 'AI 直接跑在宿主机上，不进容器 —— 它管得了整台机器',
            brandPoint2: '每一次变更都在浏览器上确认，看得懂的一句话，不是一段 JSON',
            brandPoint3: '单节点、开源、可自建',
            brandFork: '基于 1Panel 的改动版本 · GPL-3.0 · 与飞致云无隶属关系',
            permFirstUse: '并授权本会话使用「{0}」板块。',
            permConfirmAgain: '再点一次确认',
            permDanger: '危险操作，执行后不可恢复。',
            permRawInput: '原始入参',
            permModuleHint: '允许后，本会话内这个板块的只读操作不再询问；变更和删除仍会逐次确认。不跨会话记忆。',
            permOpCount: '{0} 个操作',
            permDestructiveCount: '其中 {0} 个为删除类',
            mcpTitle: '面板操作能力（MCP）',
            mcpHint: '让 agent 能操作这个面板：应用、网站、证书、容器。每一次变更仍然会先问你。',
            mcpMissing: 'vipanel-mcp 未安装，面板操作能力不可用。',
            mcpState: '已就绪 {0} 个操作',
            ...
            settings: '设置',
            poolSize: '同时运行的 Agent 实例上限',
            poolHint: '会话数不受限制。超过上限时最久未用的会话会休眠，重新打开时接回原对话，历史不丢。',
            harnesses: 'Harness',
            capChat: '支持结构化聊天',
            capTermOnly: '仅终端',
            save: '保存',
